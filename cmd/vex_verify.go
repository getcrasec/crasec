...
var (
	vexVerifyCertIdentity        string
	vexVerifyCertIdentityRegex   string
	vexVerifyCertOIDCIssuer      string
	vexVerifyCertOIDCIssuerRegex string
)

var vexVerifyCmd = &cobra.Command{
	Use:   "verify <vex.cdx.json> <vex.cdx.json.sig>",
	Short: "Verify a Sigstore signature over a VEX document",
	Long: `Verify a Sigstore bundle produced by "crasec vex sign": checks the
...
func init() {
	vexCmd.AddCommand(vexVerifyCmd)
	vexVerifyCmd.Flags().StringVar(&vexVerifyCertIdentity, "certificate-identity", "", "require the signing certificate SAN to equal this value")
	vexVerifyCmd.Flags().StringVar(&vexVerifyCertIdentityRegex, "certificate-identity-regex", "", "require the signing certificate SAN to match this regex")
	vexVerifyCmd.Flags().StringVar(&vexVerifyCertOIDCIssuer, "certificate-oidc-issuer", "", "require the signing certificate's OIDC issuer to equal this value")
	vexVerifyCmd.Flags().StringVar(&vexVerifyCertOIDCIssuerRegex, "certificate-oidc-issuer-regex", "", "require the signing certificate's OIDC issuer to match this regex")
}

func runVexVerify(cmd *cobra.Command, args []string) error {
	artifactPath, sigPath := args[0], args[1]
	out := cmd.OutOrStdout()

	var identity *artifactsign.Identity
	if vexVerifyCertIdentity != "" || vexVerifyCertIdentityRegex != "" || vexVerifyCertOIDCIssuer != "" || vexVerifyCertOIDCIssuerRegex != "" {
		identity = &artifactsign.Identity{
			SAN:         vexVerifyCertIdentity,
			SANRegex:    vexVerifyCertIdentityRegex,
			Issuer:      vexVerifyCertOIDCIssuer,
			IssuerRegex: vexVerifyCertOIDCIssuerRegex,
		}
	} else {
